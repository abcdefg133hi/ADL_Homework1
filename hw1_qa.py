...
def main():
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        print(f"loss in epoch {epoch} is {train_loss}")
        # Evaluation
        logger.info("***** Running Per Evaluation *****")
        ...
        logger.info(f"Epoch {epoch},  with evaluation metrics: {eval_metric}")
        ...
        for step, batch in enumerate(train_dataloader):
            with torch.no_grad():
                outputs = model(**batch)
                astart_logits = outputs.start_logits
                aend_logits = outputs.end_logits
                loss = outputs.loss
                total_val_loss += loss.detach().float()
                if not args.pad_to_max_length:  # necessary to pad predictions and labels for being gathered
                    astart_logits = accelerator.pad_across_processes(astart_logits, dim=1, pad_index=-100)
                    aend_logits = accelerator.pad_across_processes(aend_logits, dim=1, pad_index=-100)

                aall_start_logits.append(accelerator.gather_for_metrics(astart_logits).cpu().numpy())
                aall_end_logits.append(accelerator.gather_for_metrics(aend_logits).cpu().numpy())

        max_len = max([x.shape[1] for x in aall_start_logits])  # Get the max_length of the tensor

        # concatenate the numpy array
        start_logits_concat = create_and_fill_np_array(aall_start_logits, train_dataset, max_len)
        end_logits_concat = create_and_fill_np_array(aall_end_logits, train_dataset, max_len)

        # delete the list of numpy arrays
        del aall_start_logits
        del aall_end_logits

        outputs_numpy = (start_logits_concat, end_logits_concat)
        prediction = post_processing_function(train_examples, train_dataset, outputs_numpy)
        eval_metric = metric.compute(predictions=prediction.predictions, references=prediction.label_ids)
        logger.info(f"Epoch {epoch},  with training evaluation metrics: {eval_metric}")
        """

    # Evaluation
    logger.info("***** Running Total Evaluation *****")
    logger.info(f"  Num examples = {len(eval_dataset)}")
    logger.info(f"  Batch size = {args.per_device_eval_batch_size}")
